...
- **Linux-KVM-Gruppe kann im Agent-Prozess fehlen, obwohl `/etc/group` den Nutzer
  auffuehrt:** Bei `Permission denied` auf `/dev/kvm` den Emulator mit
  `sg kvm -c '...emulator...'` starten. Die aktuelle Shell erbt nach einer
  Gruppen-Aenderung nicht automatisch die Zusatzgruppe. (29.7.)
- **MAUI DevFlow kann inaktive Shell-Seiten samt Kindern als `isVisible=true`
  fuehren:** Nach Push/Pop oder Deep-Link koennen AutomationIds doppelt vorkommen;
  `ui scroll --element <AutomationId>` trifft dann den falschen Knoten oder scheitert.
  Aktuelle Page, Bounds und echten Android-Viewport gegenpruefen; fuer sichtbaren Text
  zusaetzlich `uiautomator dump` verwenden. (29.7.)
- **DevFlow-Window-Screenshot kann ein natives Android-AlertDialog auslassen:** Der
  Dialog wird von `ui alert detect/tree` gefunden, im `ui screenshot` ist aber nur
  das abgedunkelte App-Fenster. Fuer Dialog-Screenshots
  `adb exec-out screencap -p` verwenden. (29.7.)
- **`maui devflow network detail` gibt Authorization-Header unmaskiert aus:** Nur
  gezielt lokal inspizieren und niemals den Output mit Bearer-Token in Berichte
  uebernehmen. (29.7.)
...
- **test.seller:** Stand 29.7. zwei Inserate (`Modernes Reihenhaus in Wels`,
  `Grosses Baugrundstueck Muehlviertel`), normalerweise keine Entwuerfe
  (`/api/property-drafts/` leer).
