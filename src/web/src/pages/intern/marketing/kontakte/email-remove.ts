...
/**
 * PRG-Action: Zusatzadresse eines Kontakts entfernen. Bereits zugeordnete Mails behalten
 * ihren Kontakt - entfernt wird nur die kuenftige Zuordnung ueber diese Adresse.
 */
export const POST: APIRoute = async ({ request }) => {
  ...
  const result = await adminApiPost<MarketingContactEmailActionResponse>(
    "/api/admin/marketing/contacts/emails/remove",
    {
      ContactId: contactId,
  ...
  if (result?.Success) {
    params.set("action", "emailremoved");
  } else {
