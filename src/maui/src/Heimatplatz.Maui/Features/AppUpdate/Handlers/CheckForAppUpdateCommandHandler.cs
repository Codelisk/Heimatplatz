...
namespace Heimatplatz.Maui.Features.AppUpdate.Handlers;
    ...
    : ICommandHandler<CheckForAppUpdateCommand>
{
    ...
    {
        updateService.UpdateDownloaded -= OnUpdateDownloaded;
        _ = CompleteUpdateSafeAsync();
    }

    private async Task CompleteUpdateSafeAsync()
    {
        try
        {
        ...
        {
            logger.LogError(ex, "Failed to complete downloaded update");
        }
    }
}
